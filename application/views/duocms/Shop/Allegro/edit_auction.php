...
<div class="panel panel-default">
    <div class="panel-heading">
        <div class="row">
            <div class="col-sm-12">
                <strong>Edycja aukcji <?= $allegro_auction_id; ?></strong>
    ...
    <div class="panel-body">
        ...
        <form method="POST">
            <input type="hidden" name="allegro_id" value="<?= $allegro_auction_id; ?>" />
            ...
            <p><strong>Opis</strong></p>
                    ...
                    <input type="hidden" name="description[sections][0][items][0][type]" value="TEXT" />
                        <textarea name="description[sections][0][items][0][content]" class="form-control" rows="12">
                            <?= empty($acution_data['description']['sections'][0]['items'][0]['content']) ? strip_tags($product_translation->body, '<p><ul><li><h1><h2>') : $acution_data['description']['sections'][0]['items'][0]['content']; ?>
                        </textarea>
                    </div>
                    <div class="col-sm-6" style="height:300px;">
            ...
            <select name="parameters[<?= $p;?>][valuesIds][]" class="form-control">
                ...
                if(!empty($field->dictionary)){
                    foreach ($field->dictionary as $key=>$opt){
                        $selected_string = '';
                        if(!empty($fval)){
                            $selected_string = ($opt->id == $fval) ? 'selected' : '';
                        } else {
                            $selected_string = (in_array($opt->id, $usedAttributes)) ? 'selected' : '';
                        }
                        ?>
                <option value="<?= $opt->id;?>" <?= $selected_string;?>><?= $opt->value;?></option>
                <?php
                    }
                }
                ?>
            ...
            <div class="form-group">
                ...
                <select name="publication[duration]" class="form-control">
                    ...
                    <option value="null" selected="selected">Do wyczerpania zapasów</option>
            ...
            <div class="form-group">
                ...
                <select name="delivery[handlingTime]" class="form-control">
                    ...
                    <option value="PT24H" selected="selected">24 godziny</option>
                    <option value="P7D">7 dni</option>
            ...
            <div class="form-group">
                ...
                <select name="payments[invoice]" class="form-control">
                    ...
                    <option value="WITHOUT_VAT" selected="selected">Faktura bez vat</option>
...
<!--            <input type="hidden" name="createdAt" value="<?= date('Y-m-d').'T'.date('H:i:s').'Z';?>" />
            ...
            <div class="form-group">
                <input type="submit" value="Edytuj aukcję" class="btn btn-success" />
            </div>
            <?php
            } else {
