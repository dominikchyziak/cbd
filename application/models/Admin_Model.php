...
class Admin_Model extends MY_Model {
    ...
    public static function loggedin() {
        ...
        if (!is_object(self::$loggedin)) {
            self::$loggedin = Admin_Model::factory(
                            $CI->session->userdata('admin')['user']['id']
            );
        }
    ...
    public static function isLoggedIn() {
        return Admin_Model::loggedin()->id > 0;
    }
    ...
    public function add_user($username, $password, $first_name = "", $last_name = "", $phone = "", $city = "", $zip_code = "", $street = "", $building_number = "", $type = "0", $status = "0", $discount = 0, $newsletter = 0) {
        ...
        $password = md5($password);
        $code = sha1($this->db->insert_id() . $password);
